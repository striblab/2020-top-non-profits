...
module.exports = {
  ...
  nonprofits: {
    source: `${
      ...
      config.data ? config.data.publishYear.data : '2020'
...
function parseNonprofits(input) {
  ...
  let publishYear = config.data ? config.data.publishYear.data : 2020;
  let validYears = [publishYear, publishYear - 1];
  let parsed = input.objects;

  // Ohh, this should be the 2019 list ...
  let list2019 = JSON.parse(
    fs.readFileSync(
      path.join(__dirname, '../sources/2019-non-profit-list.json')
    )
  );
  ...
  // Rank previous year
  parsed = _.map(parsed, p => {
    let previous = _.find(list2019, { coid: p.coid });
    if (previous) {
